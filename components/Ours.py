...
class OSA_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(self.layer[:2])
        out1 = self.layer[:1](x)
        local_maps.append(out1)

        out = self.layer[1:](out1)
        MPB_maps.append(out)
...
class OSAG(nn.Module):
    def __init__(self, channel_num=64, bias=True, block_num=1, window_size=16, ffn_bias=True, pe=True):
...
class JRFAB(nn.Module):
    ...
    def forward(self, lr, ref):
        ref = torch.nn.functional.interpolate(ref, scale_factor=1/self.scale, mode='bilinear', align_corners=False)
...
class OmniSR(nn.Module):
    def __init__(self, num_in_ch=3, num_out_ch=3, num_feat=64, up_scale=4, res_num=5, bias=True, window_size=16, block_num=1, pe=True, ffn_bias=True):
        ...
        for _ in range(res_num):
            temp_res = OSAG(channel_num=num_feat, window_size=window_size, block_num=block_num, pe=True, ffn_bias=True)
            residual_layer.append(temp_res)
            # fus.append(JRFAB(num_feat,up_scale))
        self.residual_layer = nn.Sequential(*residual_layer)
        # self.featureFusions = nn.Sequential(*fus)
        self.input = nn.Conv2d(in_channels=num_in_ch, out_channels=num_feat, kernel_size=3, stride=1, padding=1, bias=bias)
        self.input2 = nn.Conv2d(in_channels=num_in_ch, out_channels=num_feat, kernel_size=3, stride=1, padding=1, bias=bias)
        self.output = nn.Conv2d(in_channels=num_feat, out_channels=num_feat, kernel_size=3, stride=1, padding=1, bias=bias)
        self.up = pixelshuffle_block(num_feat, num_out_ch, up_scale, bias=bias)
        self.featureFusion = JRFAB(num_feat,up_scale)
        # self.last = nn.Conv2d(in_channels=num_feat, out_channels=num_out_ch, kernel_size=3, stride=1, padding=1, bias=bias)
        self.window_size = window_size
    ...
    def forward(self, x, y):
        ...
        for num,blk in enumerate(self.residual_layer):
            x1 = blk(x1)
            out = self.featureFusion(x1, y1)
            featureFusion_maps.append(out)
            z += out
        z_maps.append(z)
        return self.up(z)
...
if __name__ == '__main__':
    ...
    # y = torch.nn.functional.interpolate(x, scale_factor=0.25, mode='bilinear', align_corners=False)
    # print(y.shape)
    model = OmniSR(1, 1, 64, up_scale=4, res_num=4, block_num=1, bias=True,
    ...
    #     # 选择要展示的通道
    #     num_channels = 64
    #     selected_features = features[:num_channels]
    #
    #     # 创建网格
    #     grid = make_grid(selected_features.unsqueeze(1), nrow=8, normalize=True)
    #     # grid = make_grid(selected_features.unsqueeze(1), nrow=4)
    #
    #     # 转换为可展示的格式
...
# Model FLOPs:  7.078G
# Model Params: 817.936K

# Model FLOPs:  7.022G
# Model Params: 811.012K