...
class Trainer(object):
    def __init__(self, config, reporter):
        ...
        # ================urban100======================#
        self.eval_loader1 = dataloaderClass('ixi',
                                            config["test_dataset_paths"]['ixi'],
    ...
    def __evaluation__(self, eval_loader, eval_iter, epoch, step=0,current_lr=0):
        ...
        with torch.no_grad():
            for _ in tqdm(range(eval_iter)):
                hr, lr, ref = eval_loader()
                # print(lr.shape)
                if self.config["cuda"] >= 0:
                    hr = hr.cuda()
                    lr = lr.cuda()
                    ref = ref.cuda()
                res = self.network(lr, ref)
    ...
    def train(self):
        ...
        for epoch in range(start, total_epoch):
            ...
            for step in range(step_epoch):
                ...
                hr, lr, ref = self.train_loader.next()

                generated_hr = self.network(lr, ref)
