...
class DataPrefetcher():
    ...
    def __preload__(self):
        try:
            self.hr, self.lr, self.ref = next(self.dataiter)
...
class IXI_train(data.Dataset):
    def __init__(self, ixi_root, scale, crop_size, **kwargs):
        ...
        self.scale = scale
        assert len(hr_t2s) == len(lr_t2s), 'ref image number != hr image number'
        print('total train image number:%s' % len(lr_t2s))
